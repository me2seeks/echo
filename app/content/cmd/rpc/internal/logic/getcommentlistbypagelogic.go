...
func (l *GetCommentListByPageLogic) GetCommentListByPage(in *pb.GetCommentListByPageReq) (*pb.GetCommentListByPageResp, error) {
	...
	var err error
	if !in.IsComment {
		commentList, total, err = l.svcCtx.CommentsModel.FindPageListByPageWithTotal(l.ctx, l.svcCtx.CommentsModel.SelectBuilder().Columns("*").Where("feed_id = ?", in.Id), in.Page, in.PageSize, "")
		if err != nil {
		...
		commentList, total, err = l.svcCtx.CommentsModel.FindPageListByPageWithTotal(l.ctx, l.svcCtx.CommentsModel.SelectBuilder().Columns("*").Where("parent_id = ?", in.Id), in.Page, in.PageSize, "")
		if err != nil {
